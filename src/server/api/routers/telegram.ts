...
export const telegramRouter = createTRPCRouter({
  ...
  session: publicProcedure
    .input(z.object({ telegramUserId: z.string().or(z.number()) }))
    .mutation(async ({ input, ctx }) => {
  ...
  updateSession: publicProcedure
    .input(z.object({ sessionId: z.string(), contractAddressId: z.string() }))
    .mutation(async ({ input, ctx }) => {
      console.log("getSession called with sessionId:", input.sessionId);
      const session = await ctx.db.session.findFirstOrThrow({
        where: { id: input.sessionId },
      ...
      await Promise.all([
        ctx.db.user.update({
          where: { id: user.id },
          data: {
            defaultContractAddress: input.contractAddressId,
          },
        }),
        ctx.db.session.update({
          where: { id: session.id },
          data: {
            contractAddress: input.contractAddressId,
          },
        }),
      ]);
    }),
